...
const initDatabase = async () => {
  try {
    ...
    if (keyspaceMetadata.rows.length === 0) {
      ...
      await cassandraAdminClient.execute(CREATE_KEYSPACE_QUERY);
      console.log(`Created keyspace ${DATABASE_KEYSPACE}`);
    }

    // Create user table
    console.log(`Creating table user in keyspace ${DATABASE_KEYSPACE}`);
    await cassandraClient.execute(CREATE_USER_TABLE_QUERY);
    ...
    await cassandraClient.execute(CREATE_TODO_TABLE_QUERY);
